...
const PurchaseItems = () => {
  ...
  useEffect(() => {
    ...
    const stockValue = items.reduce((sum, item) => sum + (parseFloat(item.unitCost) * item.stock), 0);
  ...
  return (
    <div className="min-h-screen flex">
        ...
        <Content className="transition-all duration-300 p-6">
          ...
          >
            <Form form={form} onFinish={handleSave} layout="vertical">
              <Row gutter={16}>
                <Col xs={24} md={8}>
                  <Form.Item name="supplier" label="Supplier *" rules={rules.required}>
                ...
                <Col xs={24} md={8}>
                  <Form.Item name="purchaseDate" label="Purchase Date *" rules={rules.required}>
                    <DatePicker className="w-full" format="DD-MM-YYYY" />
                  </Form.Item>
                </Col>
                <Col xs={24} md={8}>
                  <Form.Item name="invoiceNumber" label="Invoice Number *" rules={rules.required}>
              ...
              <Row gutter={16}>
                <Col xs={24} md={8}>
                  <Form.Item name="orderType" label="Order Type *" rules={rules.required}>
              ...
              <Row justify="end" className="mb-4">
                <Col xs={24} md={8}>
                  <div className="bg-white shadow-md p-6 rounded-lg w-full flex flex-col">
                    <div className="flex justify-between mb-4 text-sm font-medium text-gray-700">
                      <Text>Subtotal:</Text>
                      <Text className="font-semibold text-gray-900">Ksh {totals.subtotal.toFixed(2)}</Text>
                    </div>
                    <div className="flex justify-between mb-4 text-sm font-medium text-gray-700">
                      <Text>Round Off:</Text>
                      <Text className="font-semibold text-gray-900">Ksh {totals.roundOff.toFixed(2)}</Text>
                    </div>
                    <div className="flex justify-between mb-4 text-sm font-medium text-gray-700">
                      <Text className="font-semibold">Grand Total:</Text>
                      <Text className="font-semibold text-xl text-green-600">Ksh {totals.grandTotal.toFixed(2)}</Text>
                    </div>
                    <div className="flex justify-between text-sm font-medium text-gray-700">
                      <Text>Stock Value:</Text>
                      <Text className="font-semibold text-gray-900">Ksh {totals.stockValue.toFixed(2)}</Text>
                    </div>
                  </div>
                </Col>
              </Row>
              ...
              <Form.Item>
                <div className="flex justify-end space-x-2">
                  <Button
                    onClick={() => router.push('/purchase_history')}
                    className="bg-blue-500 text-white hover:bg-blue-600 rounded-lg px-4 py-2 mr-2"
                  >
      ...
      <Modal
        ...
        footer={null}
      >
        <AddItemForm onSubmit={handleAddItem} onCancel={() => setItemModalOpen(false)} />
      </Modal>
