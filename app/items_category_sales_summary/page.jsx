...
const mockCategorySalesData = [
  {
    ...
    returnRate: 2,
    discount: 10,
    date: "2025-01-25", // Added date field
  },
  {
    id: "C002",
    category: "Sugar Bags",
    ...
    returnRate: 5,
    discount: 5,
    date: "2025-01-28", // Added date field
  },
  {
    id: "C003",
    category: "Sugar",
    ...
    returnRate: 1,
    discount: 0,
    date: "2025-01-30", // Added date field
  },
  {
    id: "C004",
    category: "Soup",
    ...
    returnRate: 3,
    discount: 15,
    date: "2025-01-22", // Added date field
  },
  {
    id: "C005",
    category: "Test",
    ...
    returnRate: 0,
    discount: 0,
    date: "2025-01-29", // Added date field
  },
];
...
const ItemsCategorySalesReport = () => {
    ...
    setTimeout(() => {
      ...
      if (filteredData.length === 0) {
        notificationApi.warning({
          ...
          description: "No category sales data found for the selected date range.",
          placement: "topRight",
  ...
  const exportToPDF = () => {
    ...
    doc.text("Category Sales Report", 70, 30);
    ...
    doc.setFont("helvetica", "bold");
    doc.text(" Category Sales Report Information", margin.left + 5, 60);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    ...
    doc.autoTable({
      startY: 100,
      head: [["Category", "Item Code/Name", "Item Qty", "Total Sales (KES)"]],
    ...
    addFooter();
    doc.save(`category_sales_report_${dayjs().format("DD-MM-YYYY")}.pdf`);
  ...
  const groupedData = filteredData.reduce((acc, item) => {
    ...
    acc[item.category].totalQty += item.itemQty;
    acc[item.category].totalSales += item.totalSales;
    acc[item.category].profitMargin += item.profitMargin;
    acc[item.category].returnRate += item.returnRate;
    acc[item.category].discount += item.discount;
    return acc;
  }, {});

  // Flatten the grouped data for the table
  const tableData = Object.keys(groupedData).flatMap((category) => {
    const categoryData = groupedData[category];
    return [
      ...categoryData.items,
      {
        id: `total-${category}`,
        category: `Subtotal for ${category}`, // Changed from "Total for [Category]" to "Subtotal for [Category]"
        itemCode: "",
        itemName: "",
  ...
  const collapseItems = [
    {
      ...
      children: (
        <>
          ...
          <div className="mb-4">
            <Input
              placeholder="Search by Category, Item Code, or Item Name"
          ...
          <Row gutter={16} className="mt-4">
            <Col span={12}>
              <Card title="Total Sales by Category" className="shadow-sm">
                <Bar data={categorySalesChartData} />
              </Card>
            </Col>
            <Col span={12}>
              <Card title="Profit Margin by Category" className="shadow-sm">
