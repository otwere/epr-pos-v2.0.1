...
const CreditorReport = () => {
  ...
  // Function to calculate aging analysis for payables
  ...
  const generateReport = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      // Calculate aging analysis dynamically for each creditor
      const updatedCreditorsData = mockCreditorsData.map((creditor) => ({
        ...creditor,
        agingAnalysis: calculateAgingAnalysis(creditor.invoices),
      }));

      setCreditorsData(updatedCreditorsData);
      setLoading(false);
      setIsReportVisible(true);
      setReportGeneratedTime(dayjs().format("DD-MM-YYYY HH:mm:ss"));
      notificationApi.success({
        message: "Report Generated",
        description: "Creditors Report generated successfully.",
        placement: "topRight",
  ...
  const exportToPDF = () => {
    ...
    doc.text("Creditors Report", 70, 30);
    ...
    doc.setFont("helvetica", "bold");
    doc.text(" Creditors Report Information", margin.left + 5, 60);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    ...
    addFooter();
    doc.save(`creditors_report_${dayjs().format("DD-MM-YYYY")}.pdf`);
  ...
  const filteredData = creditorsData.filter((item) => {
    return (
      item.creditorName.toLowerCase().includes(searchText.toLowerCase()) ||
  ...
  const totalPayables = creditorsData.reduce(
      ...
      children: (
        <>
          <Row gutter={16} className="mb-4">
            <Col span={24} key="total-payables">
          ...
          <div className="mb-4">
            <Input
              placeholder="Search by Creditor Name or Contact Details"
